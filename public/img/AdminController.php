...
class AdminController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $user = DB::table('admin')
                ->where('name','=', $request->input('login_username'))
                ->first();
        if($user){
    ...
    public function showCampusReports(Request $request)
    {
        ...
        $result = DB::table('users As U')
                  ->join('test_completion_details As TD','TD.user_id','=','U.id')
                  // ->groupBy('U.campus_id')
                  // ->where('U.campus_id',$camp->campus_id)
                  ->get();
                ...
                // print_r($result);
                // exit;
                $pass = DB::table('test_completion_details As tcd')
                ->select(DB::raw('count(tcd.id) As passcount'),'cd.campus_id')
                ->join('users As u','tcd.user_id','=','u.id')
                ->join('campus_details As cd','cd.campus_id','=','u.campus_id')
                ->where('result','1')
                ->groupBy('cd.campus_id')
                ->get();
                // echo "<pre>";
                // print_r($pass);
                $fail = DB::table('test_completion_details As tcd')
                ->select(DB::raw('count(tcd.id) As failcount'),'cd.campus_id')
                ->join('users As u','tcd.user_id','=','u.id')
                ->join('campus_details As cd','cd.campus_id','=','u.campus_id')
                ->where('result','0')
                ->groupBy('cd.campus_id')
                ...
                // print_r($fail);
                // exit;
                $considered = DB::table('test_completion_details As tcd')
                ->select(DB::raw('count(tcd.id) As considered'),'cd.campus_id')
                ->join('users As u','tcd.user_id','=','u.id')
                ->join('campus_details As cd','cd.campus_id','=','u.campus_id')
                ->where('result','2')
                ->groupBy('cd.campus_id')
                ...
                // print_r($considered);
                // exit;
        $dates = DB::table('campus_details')
                ->select(DB::raw('YEAR(campus_date) AS year'))
                ->groupBy('year')
                ->get();
                // echo "<pre>";
                // print_r($dates);
        ...
        $campus_count = Campus::all()->count();
        // $user_count = User::all()->count();
        $user_count = DB::table('test_completion_details')
                      ->select(DB::raw('count(user_id) As user_count'))
                      ->get();
                      // print_r($user_count[0]->user_count);
                      // exit;
    ...
    public function showUserReports(Request $request,$id)
    {
        ...
        if($request->session()->has('user_id') && session('user_name') == 'admin'){
            ...
            $data['reports'] = DB::table('test_completion_details As TCD')
                                ->join('users As U','U.id', '=', 'TCD.user_id')
                                ->join('campus_details As CD','CD.campus_id','=','U.campus_id')
    ...
    public function showUserDetailView(Request $request,$user_id = '')
    {
        if($request->session()->has('user_id') && session('user_name') == 'admin')
        {
                ...
                $data['results'] = DB::table('test_completion_details As TCD')
                ->join('users As U','U.id', '=', 'TCD.user_id')
    ...
    public function forget_password(Request $request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if($email_exists)
            {
                ...
                return redirect('/forgot-pwd')
                        ->withErrors(['Email Id does not exists in our records']);
            }
    ...
    public function dashboard(Request $request) {
      ...
      // die;
        ...
        if($request->session()->has('user_id') && session('user_name') == 'admin'){
            ...
            $user =  DB::table('users')
                        ->select('campus_details.campus_name',DB::raw('count(users.id) AS total_users'))
                        ->join('campus_details',
                            'users.campus_id','=','campus_details.campus_id')
                        ->groupBy('campus_details.campus_id')
                        ->get();
            //echo "<pre>";
            // print_r($user);
            // exit;
   ...
   public function update_result(Request $request)
    {
        ...
        // echo $chckId;
        // exit();
        $result = DB::table('test_completion_details')->where('user_id', $chckId)
                 ->update(['result'=>'2']);
        if($result){
